...
TEST( MetaOperationTest, testInvoke )
{
    nbe::Lua lua;

    lua.execute("etc/assets/tests/MetaOperation/testFunction.lua");
    lua_getglobal(lua.get(),"testFunction");
    nbe::MetaOperation * op = new nbe::MetaOperation(lua, 0, 1);
    ASSERT_TRUE( op->ok() ) << "There are no errors after constructing a MetaOperation";
    op->invoke();
    ASSERT_EQ(1,lua_tointeger(lua.get(),-1));
    lua_pop(lua.get(),1);
    delete op;
}

TEST( MetaCoroutineTest, testConstructFailedToMoveFunction )
{
    nbe::Lua lua;

    lua.execute("etc/assets/tests/MetaOperation/testNonExistent.lua");
    lua_getglobal(lua.get(),"testCoroutineYield");
    nbe::MetaCoroutine * op = new nbe::MetaCoroutine(lua);
    EXPECT_EQ( false, op->ok() ) << "There is an error after trying to construct with a non-existent function";
    EXPECT_EQ( nbe::MetaCoroutine::FailedToMoveFunction, op->error() );
    delete op;
}

TEST( MetaCoroutineTest, testResumeYield )
{
    nbe::Lua lua;

    lua.execute("etc/assets/tests/MetaOperation/testCoroutine.lua");
    lua_getglobal(lua.get(),"testCoroutineYield");
    nbe::MetaCoroutine * op = new nbe::MetaCoroutine(lua);
    ASSERT_TRUE( op->ok() ) << "There are no errors after constructing a MetaCoroutine";
    int nres = 0;
    int result = op->resume(&nres);
    ASSERT_EQ(1, nres);
    if ( result == LUA_ERRRUN )
    {
        std::cerr << "Error:" << lua_tostring(op->thread(), -1) << '\n';
    }
    ASSERT_EQ(LUA_YIELD, result);
    ASSERT_EQ(1,lua_gettop(op->thread()));
    ASSERT_EQ(1,lua_tointeger(op->thread(),1));
    lua_pop(op->thread(),1);
    delete op;
}

TEST( MetaCoroutineTest, testResumeComplete )
{
    nbe::Lua lua;

    lua.execute("etc/assets/tests/MetaOperation/testCoroutine.lua");
    lua_getglobal(lua.get(),"testCoroutineComplete");
    nbe::MetaCoroutine * op = new nbe::MetaCoroutine(lua);
    ASSERT_TRUE( op->ok() ) << "There are no errors after constructing a MetaCoroutine";
    int nres = 0;
    int result = op->resume(&nres);
    ...
    if ( result == LUA_ERRRUN )
    {
        std::cerr << "Error:" << lua_tostring(lua.get(), -1) << '\n';
    }
    ASSERT_EQ(LUA_OK, result);
    ASSERT_EQ(1,lua_tointeger(op->thread(),1));
    lua_pop(op->thread(),1);
    delete op;
}
